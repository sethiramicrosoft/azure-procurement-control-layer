...
function createManagedStore({ managedAdapterPath, exportAudit }) {
  if (!managedAdapterPath) {
    throw new Error('APCL_MANAGED_STATE_ADAPTER_PATH is required when APCL_STATE_BACKEND=managed');
  }
  const adapterPath = path.resolve(managedAdapterPath);
  const adapter = require(adapterPath); // eslint-disable-line global-require, import/no-dynamic-require
  const required = ['loadState', 'saveState', 'tryConsumeEntitlement'];
  const missing = required.filter(name => typeof adapter[name] !== 'function');
  if (missing.length) {
    throw new Error(`Managed state adapter missing required functions: ${missing.join(', ')}`);
  }
  return {
    loadState: adapter.loadState,
    saveState: adapter.saveState,
    tryConsumeEntitlement: adapter.tryConsumeEntitlement,
    exportAudit: typeof adapter.exportAudit === 'function' ? adapter.exportAudit : exportAudit,
  };
}

function createStateStore({ dataDir, dataFile, sqliteDbPath, backend, auditExportPath, auditExportSecret, managedAdapterPath }) {
  ...
  if (selected === 'managed') {
    return createManagedStore({ managedAdapterPath, exportAudit });
  }
